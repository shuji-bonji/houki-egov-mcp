...
**minor リリース** — 施行令・施行規則の関連付けと、条文本文からの参照抽出のツールを 2 つ足した（Issue #20）。houki-hub#8（法令グラフ）と対象が重なるが、MCP は法令 XML と法令名の規則から決定論的に引ける参照だけを返す、という分担（houki-hub `docs/ROADMAP.md` 2026-09-14 決定）。設計メモは houki-hub `docs/notes/2026-09-19-design-egov-20-references.md`。
