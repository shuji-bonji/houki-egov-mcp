# プロダクト開発における houki-hub-mcp ユースケース集

「請求書を電子化して」「本人確認を実装して」のようなオーナーからの実装依頼の背後には、複数省庁にまたがる法令調査が潜んでいます。各ユースケースで houki-hub-mcp が**どのレイヤー**を提供し、**どこから先は有資格者の領域**かを示します。
...
### houki-hub-mcp での取得経路

| レイヤー | houki-hub-mcp ツール | データ取得元 |
|---|---|---|
...
| 取扱通達 | `nta_search({ type: "tsutatsu", keyword: "電帳法" })` | `@houki-hub/ext-nta`（Phase 3） |
| 一問一答 Q&A | `nta_search({ type: "qa", keyword: "電帳法" })` | `@houki-hub/ext-nta`（Phase 3） |
...
| 三省共同 解釈通達 | `soumu_search` / `moj_search` / `meti_search` | `@houki-hub/ext-{soumu,moj,meti}`（Phase 3） |
...
| 個情委ガイドライン・Q&A | `ppc_search({ type: "guideline" })` | `@houki-hub/ext-ppc`（Phase 3） |
...
| 金融庁ガイドライン | `fsa_search({ keyword: "本人確認" })` | `@houki-hub/ext-fsa`（Phase 3） |
...
│ LLM           事実からの分析・論点整理 │
│  ↑                                   │
│ houki-hub-mcp 一次情報の取得・提示のみ │
└─────────────────────────────────────┘
```

**houki-hub-mcp は「叩き台のコストを下げる」道具**。最終判断と責任は利用者と有資格者にあります。詳細は [DISCLAIMER.md](../DISCLAIMER.md) 参照。
