...
| 国税庁告示 | `get_law(...)` または NTA直接 | e-Gov 一部 + `houki-nta-mcp`（計画中） |
| 取扱通達 | `nta_search({ type: "tsutatsu", keyword: "電帳法" })` | `houki-nta-mcp`（計画中） |
| 一問一答 Q&A | `nta_search({ type: "qa", keyword: "電帳法" })` | `houki-nta-mcp`（計画中） |
...
| 認定基準告示 | `get_law(...)` | e-Gov 一部 + `houki-soumu-mcp`（計画中） |
| 三省共同 解釈通達 | `soumu_search` / `moj_search` / `meti_search` | `houki-soumu-mcp` / `houki-moj-mcp` / `houki-meti-mcp`（計画中） |
...
| 個情委ガイドライン・Q&A | `ppc_search({ type: "guideline" })` | `houki-ppc-mcp`（計画中） |
| 業法独占規定の確認 | （v0.2.0 で削除。後日 `houki-research` Skill として再構築予定） | Skill 層（計画中） |
...
| 金融庁ガイドライン | `fsa_search({ keyword: "本人確認" })` | `houki-fsa-mcp`（計画中） |
...
### 「3ステップ」で houki-hub family を使う

1. **`explain_law_type`**（houki-egov-mcp）で**地形把握**（このレイヤーは何？拘束力は？）
2. **`get_law` / `search_law` / `get_toc` / `get_law_revisions`**（houki-egov-mcp）で**条文取得**（事実情報の収集）
3. **他 MCP の `{namespace}_search` / `{namespace}_get`**（`houki-nta-mcp` / `houki-mhlw-mcp` 等、計画中）で**実務基準の補完**（通達・Q&A・ガイドライン）

業法独占規定（弁護士法72条等）への注意は、後日 `houki-research` Skill として再構築されます（v0.1.x で houki-egov-mcp に同梱されていた `explain_business_law_restriction` ツールは v0.2.0 で削除されました）。
